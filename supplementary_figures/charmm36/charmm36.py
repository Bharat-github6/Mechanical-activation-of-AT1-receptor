...
plt.plot(time_sopc, sopc_apo[['TM1-TM6']].rolling(20).mean()*10, color='tab:blue', label='Apo AT1R in SOPC (replica1)')
plt.plot(time, sopc_apo_anton[['TM1-TM6']].rolling(10).mean()*10, color='tab:red', label='Apo AT1R in SOPC (replica2)' )
plt.plot(time_popc, popc_apo[['TM1-TM6']].rolling(20).mean()*10,color='tab:green', label='Apo AT1R in POPC')
plt.text(-30, 37,'a', weight='bold', fontsize=15)
plt.xlim(0,1000)
plt.ylim(16, 36)
plt.axhline(y=19.70, linestyle='--', color='black', lw=2)
...
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
plt.text(-30, 37,'b', weight='bold', fontsize=15)
fig = plt.gcf()
plt.subplots_adjust(top=0.92, bottom=0.1, left=0.15, right=0.95, hspace=0.3, wspace=0.0)
plt.Figure.set_size_inches(fig,(10, 8))
